...
class TestListEmployers:
    ...
    def test_skips_missing_employer_name(self, employers_dir, monkeypatch):
        (employers_dir / "noname.yaml").write_text(
            "tracks:\n  - consulting\n", encoding="utf-8"
        )
        monkeypatch.setenv("EMPLOYERS_DIR", str(employers_dir))
        from services.employer_store import EmployerEntityStore
    ...
    def test_completeness_green_when_all_required_fields_present(
        ...
        store = EmployerEntityStore()
        goldman = next(
            e for e in store.list_employers() if e["slug"] == "goldman_sachs"
        )
        assert goldman["completeness"] == "green"

    def test_normalizes_scalar_tracks_and_intake_seasons(
        self, employers_dir, monkeypatch
    ):
        (employers_dir / "drw.yaml").write_text(
            textwrap.dedent("""\
            employer_name: DRW
            slug: drw
    ...
    def test_completeness_amber_when_required_field_missing(
        self, employers_dir, monkeypatch
    ):
        # mckinsey.yaml has no intake_seasons... wait, it does. Rewrite without ep_requirement.
        (employers_dir / "partial.yaml").write_text(
            textwrap.dedent("""\
...
class TestInvalidate:
    def test_reload_after_invalidate(self, employers_dir, monkeypatch):
        ...
        (employers_dir / "new_corp.yaml").write_text(
            textwrap.dedent("""\
...
class TestToContextBlock:
    ...
    def test_explicit_employer_mention_includes_match_without_active_career_type(
        self, employers_dir, monkeypatch
    ):
        monkeypatch.setenv("EMPLOYERS_DIR", str(employers_dir))
        from services.employer_store import EmployerEntityStore
        ...
        block = store.to_context_block(
            active_career_type=None, query_text="Tell me about McKinsey Singapore"
        )
        assert "McKinsey" in block
        assert "Goldman Sachs" not in block

    def test_explicit_acronym_employer_mention_matches_single_token_name(
        self, employers_dir, monkeypatch
    ):
        (employers_dir / "dbs.yaml").write_text(
            textwrap.dedent("""\
            employer_name: DBS
            slug: dbs
            tracks:
        ...
        store = EmployerEntityStore()
        block = store.to_context_block(
            active_career_type=None, query_text="Does DBS sponsor EP?"
        )
        assert "DBS" in block
    ...
    def test_employer_missing_employer_name_handled_gracefully(
        self, employers_dir, monkeypatch
    ):
    ...
    def test_structured_facts_are_included_in_context_block(
        self, employers_dir, monkeypatch
    ):
        (employers_dir / "crypto_like.yaml").write_text(
            textwrap.dedent("""\
    ...
    def test_superseded_facts_are_filtered_from_context_block(
        self, employers_dir, monkeypatch
    ):
        (employers_dir / "history.yaml").write_text(
            textwrap.dedent("""\
    ...
    def test_structured_fact_without_key_field_uses_unnamed_fallback(
        self, employers_dir, monkeypatch
    ):
        (employers_dir / "fallback.yaml").write_text(
            textwrap.dedent("""\
...
class TestProfileTopEmployersInjection:
    def test_exact_case_insensitive_match_injects_employer(
        self, employers_dir, monkeypatch
    ):
        monkeypatch.setenv("EMPLOYERS_DIR", str(employers_dir))
        from services.employer_store import EmployerEntityStore
    ...
    def test_no_duplicate_when_employer_already_matched_by_track(
        self, employers_dir, monkeypatch
    ):
        monkeypatch.setenv("EMPLOYERS_DIR", str(employers_dir))
        from services.employer_store import EmployerEntityStore
...
class TestEmployerMatchesQueryNotes:
    ...
    def test_query_matches_employer_application_process(
        self, employers_dir, monkeypatch
    ):
        (employers_dir / "startup_co.yaml").write_text(
            textwrap.dedent("""\
    ...
    def test_single_query_term_does_not_match_notes(self, employers_dir, monkeypatch):
        (employers_dir / "general_corp.yaml").write_text(
            textwrap.dedent("""\
