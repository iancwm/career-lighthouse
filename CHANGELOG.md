...
- **Session pipeline reliability scorecard**: `docs/archived/session_pipeline_stabilization/reliability_scorecard.md` maps each observable failure mode (JSON parse/repair, alumni path, validation, append) to its UI evidence location in TraceExplorerTab.
...
- **Admin workspace header polish**: `AdminWorkspaceHeader.tsx` now uses a larger, heavier `Career Lighthouse` wordmark and removes the truncated surface description line so the shell reads more clearly at a glance.
...
- **Queue-first session debugging UX**: `SessionInbox.tsx` now splits the Staging Area into `Ready to review`, `Analyzing now`, and `Recent sessions`; promotes analyzed sessions with a `Your session is ready...` banner; keeps completed sessions under a collapsed History section; surfaces `All caught up!` and first-run empty-state guidance; and keeps `Debug Workflow` as the technical drilldown CTA.
...
- **Staging Area alumni handoff**: `AlumniDetectionModal.tsx` and `SessionInbox.tsx` now keep counsellors in the Staging Area when alumni are detected. The modal creates the session in place and reviews alumni as session cards instead of redirecting to Alumni Records and dropping the rest of the note.
...
- **Single-alumnus extraction payload shape**: `api/cfg/prompts.yaml` now requires each `alumni[]` item to be self-contained and to mirror the primary alumnus proposals for single-alumnus notes, which fixes the case where a valid extraction produced no usable alumni card.
...
- **Langfuse Redis auth healthcheck**: `docker-compose.yml` now probes the local Langfuse Redis with `redis-cli -a $LANGFUSE_REDIS_AUTH ping`, so the optional observability stack no longer reports a false unhealthy state when Redis auth is enabled.

### Changed
- **Code quality Phase 3 decomposition shipped**: `api/services/llm_tracing.py`, `api/services/llm_json.py`, and `api/services/llm_budgets.py` now own the tracing, structured-JSON, and budgeting/config helpers that used to live inside `api/services/llm.py`. `LLMTraceRecorder` now drives `_call_with_trace`, and `MergeSpec` plus `merge_chunked_results(...)` unify the chunked merge paths while keeping the existing LLM call surface intact.
...
- **Docs refreshed for the shipped refactor**: `README.md`, `docs/README.md`, `TODOS.md`, the archived code-quality finish sprint note, and the 2026-05-06 sprint-completion note now all record the verified Phase 3 `llm.py` decomposition status and the remaining follow-up work consistently.
