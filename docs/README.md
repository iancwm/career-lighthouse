...
- **Milestone 2 has shipped** (all P0/P1 tasks and P2 polish tasks in `SPRINT-M2-TASKS.md` landed, and the checked-in unit/integration tests pass): `ClaimContextService` (`api/services/claim_context.py`), the VERIFIED CLAIMS prompt injection in `chat_with_context()` (`api/services/llm.py`), `EmployerEntityStore.get_matched_slugs()` fast-path resolution, the entity-id convention fix in `entity_store.py`/`ontology_extraction.py` (P0/Task 0), Langfuse `grounding_*` trace metadata, and the gold eval query (`api/tests/test_grounding_eval.py`, `EVALUATION-PLAN.md` §7). The real-LLM eval is separately gated by the `eval` marker and `ANTHROPIC_API_KEY`. `ontology.grounding_enabled` defaults to `false` in checked-in `kb.yaml` — the pipeline ships dark. Enabling either milestone's flag for a real employer (at least 3 approved claims for a pilot employer, per `GROUNDING-DESIGN.md`'s rollout sequence) is the next decision, not further engineering.
- The active ontology rebuild sprint is tracked in `docs/ontology/REBUILD-SPRINT.md`. The remaining backlog lives in `TODOS.md`.
- The YAML rebuild MVP has shipped in `api/tools/ontology_rebuild.py`: it detects legacy YAML families, uses isolated direct Claude calls, and writes review bundles without importing into canonical stores. P0 egress/output-safety hardening, the investment-banking pilot, and the answers workflow remain open in `TODOS.md`.
- The most recently shipped work is Ontology Milestone 2 (2026-07-17, grounded chat answers) and Milestone 1 (2026-07-16, typed claims/evidence/entities) — see "Current State" above. The active sprint is the legacy YAML rebuild in `docs/ontology/REBUILD-SPRINT.md`, started 2026-07-19.
- The most recently archived top-level sprint doc is [SPRINT-SECURITY-RELIABILITY-2026-05-15.md](archived/SPRINT-SECURITY-RELIABILITY-2026-05-15.md) — all 10 security/reliability/test items shipped and archived 2026-05-20. Other archived top-level sprint docs include [SPRINT-UX-POLISH-2026-05-10.md](archived/SPRINT-UX-POLISH-2026-05-10.md), [SPRINT-UX-WORKSPACE-CLARITY.md](archived/SPRINT-UX-WORKSPACE-CLARITY.md), and [SPRINT-LAUNCH-READINESS.md](archived/SPRINT-LAUNCH-READINESS.md).
