...
| Sprint 2 — Backend shared utilities | ✓ Done (2026-04-25) | Consolidate copy-paste helpers into `shared_yaml.py` |
...
8. **S2-8** · ✓ Consolidate `_fact_payload`/`_fact_lifecycle` into `shared_yaml.py` (not `fact_store` — circular import)
