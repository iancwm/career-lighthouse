...
**Two implementation details are genuinely open, not just deferred:**

- **JSON-repair interaction with the discriminated-union payload.** The existing pipeline already has a repair path for malformed LLM JSON (`_repair_json_output`, `api/services/llm.py:499`), used today by `extract_facts_from_prose` and others. When Stage 3's LLM call returns JSON that fails `ClaimPayload`'s discriminator (`claim_type` field missing, misspelled, or not matching the payload shape actually returned), it is not yet specified whether `_repair_json_output` is invoked before or after discriminator validation, or whether the repair prompt needs to be told which claim type was expected. This needs a decision during implementation, not left to whichever behavior falls out of wiring the existing helper in unchanged — a naive wiring could either silently drop a repairable claim or repair it into the wrong payload type.
- **Multi-call cost and latency are unbudgeted.** Stages 1, 3, and 4 are each a separate LLM call (mention extraction, claim extraction, verification), versus one call for the entry point this is meant to eventually replace (`extract_facts_from_prose`). No latency budget, per-stage timeout, or non-fatal fallback is specified here — CLAUDE.md's own pre-flight checklist asks explicitly whether "any background write, index, or analysis step need[s] a timeout, retry, or non-fatal fallback," and this pipeline has three such steps where today there is one. Before Phase 2 lands (§10), each stage needs an explicit `timeout_seconds` (following the existing `settings.llm_timeout_seconds` pattern already used by `generate_alumni_extraction`) and a defined behavior on timeout — most likely: Stage 1/3 timeout aborts the extraction and returns no proposals (safe, matches "no claim without evidence"); Stage 4 (verification) timeout should **not** silently promote the claim to verified, and should instead leave it in a state the reviewer can see was unverified, so a slow verification call doesn't fail open.
...
4. Land the extraction pipeline (§5) behind the new `/extract-claims` endpoint. Feature-flaggable via a `kb.yaml` toggle (`ontology.extraction_enabled: false` by default) so it can ship dark and be exercised in staging before counsellors see it. **Note:** `api/cfg.py` loads `kb.yaml` once at process import and never re-reads it (`REPOSITORY-ASSESSMENT.md` §3a) — toggling this flag takes effect on the next deploy/restart, not on save. Plan the pilot rollout (§Phase 3 in `MIGRATION-PLAN.md`) around that, or add request-time config re-reading as a prerequisite if faster toggling is actually needed.
