...
Status: IMPLEMENTED for Milestone 1 and the Milestone 2 grounding foundation. Milestone 1 shipped 2026-07-16 and Milestone 2 shipped 2026-07-17; this document remains the living architecture spec for the dark-flagged ontology subsystem. Builds on `REPOSITORY-ASSESSMENT.md`.
...
- **`knowledge/ontology/*.yaml` are hand-maintained config placeholders, not data.** They are checked in with the Milestone 1 vocabulary, but runtime code does not read them yet. `models_ontology.py` and the code-reviewed prompt allowlists remain the source of truth while the type set is small. Runtime wiring is deferred until claim/entity types need to be extended without a code deploy.

## 3. Identifier strategy

- **`entity_id`**: `{entity_type}-{deterministic_slug}`, e.g. `organization-stripe_singapore`, `person-jane_teoh`. The generic store path derives the slug from `canonical_name` and escalates collisions. Known employer organizations use the explicit `organization-{employer_slug}` convention through `ensure_organization_entity_for_employer()`, because the filename slug can differ from a display-name slug. `aliases` and `external_ids` keep IDs stable across future renames.
...
Implement the spec's five stages as a new module, `api/services/ontology_extraction.py`, called from a **new, narrow entry point** rather than by modifying `extract_facts_from_prose`. Rationale: `extract_facts_from_prose` is wired into `employers_router.py`'s `/extract-facts` endpoint and the `EmployerFactsTab.tsx` UI today; changing its output shape would break that UI without a coordinated frontend change, which is out of scope for "no broad migration" per the spec. Milestone 1 instead adds `POST /api/kb/employers/{slug}/extract-claims` (new endpoint, additive) that runs the new pipeline over the same input (`notes` + `source_documents`) and returns `IntentCard(domain="claim")` proposals without touching the existing endpoint. Stage 1 persists idempotent `Evidence` records, and Stage 2 may persist a known-employer `Entity`; only the dedicated Stage 5 approval route writes a `Claim`.
...
- **Stage 2 (entity resolution)**: for each mention, look up candidates by exact/fuzzy name match against existing `Entity` records of the mention's inferred type (reusing `alumni_store._normalise_name_key()`-style normalization). If exactly one high-confidence match, `resolution_status="matched"`. If zero matches, `resolution_status="proposed_new"` and a *draft* `Entity` is included in the response (not written). If multiple plausible matches, `resolution_status="ambiguous"` and the claim-extraction stage **skips** that mention rather than guessing — directly satisfying "do not silently create entities when multiple plausible matches exist."
  ...
  For organization mentions, the implementation first checks the known employer store by exact normalized employer name or slug. A match creates or reuses the additive entity with the stable ID `organization-{employer_slug}`. This explicit-ID path is required because an employer filename slug is not always the same as a slug derived from its display name. Only an organization mention that matches neither an existing ontology entity nor a known employer becomes `proposed_new`.

  **This stage carries more risk than the above makes it sound, and there is already production evidence of it.** `knowledge/employers/deloitte.yaml` (slug `deloitte`, employer_name "Deloitte Singapore") and `knowledge/employers/deloitte_singapore.yaml` (slug `deloitte_singapore`) are two live, unreconciled records for what is plausibly one organization — different names, different slugs, different track taxonomies, no alias linking them (`REPOSITORY-ASSESSMENT.md` §2 item 3). Normalized-name matching, as specified above, will not reliably merge or even flag a pair like this: `"deloitte"` and `"deloitte singapore"` are not the same normalized string, and naive substring/token matching risks either missing the connection entirely (two separate `Organization` entities silently created, defeating the point of entity resolution) or over-matching (merging "Deloitte Singapore" with an unrelated "Deloitte SEA" or "Deloitte Consulting" that happens to share a token). Milestone 1's resolution bar should be set accordingly: **`resolution_status="ambiguous"` is the expected, correct outcome for name variants like this, not a failure mode to eliminate** — the pipeline should surface the ambiguity for a human to merge via `external_ids`, not attempt to resolve it automatically. `EVALUATION-PLAN.md` §2 now includes this exact pair as a gold-fixture case rather than a synthetic stand-in.
...
**Implementation decisions now closed:**

- **JSON-repair interaction with the discriminated-union payload is implemented.** Stage 3 calls `call_structured_json()` with `ClaimExtractionResult` as the validator. JSON repair happens inside that helper, and the repaired object still goes through Pydantic discriminator validation; malformed or mismatched output follows the safe no-proposal path. The behavior is covered by the ontology extraction tests.
- **Multi-call timeout and fallback behavior is implemented.** Stages 1, 3, and 4 each receive an explicit timeout. Stage 1/3 failures return no proposals, while a Stage 4 timeout returns `verification_status="unverified_timeout"` and never promotes a claim to verified. Latency and failure-rate measurement remain pilot evaluation work.

- **Stage 5 (review proposal)**: reuse the existing `IntentCard` machinery. Add `"claim"` as a fourth `IntentCard.domain` value alongside `"employer" | "track" | "alumni"`, with `diff` validated by a new `validate_intent_card_diff` branch. SmartCanvas renders the card read-mostly. Approval calls the dedicated `POST /api/kb/session/{session_id}/claims/{card_id}/commit` route, which writes the claim with `review_status="approved"`; rejection calls the existing generic discard route, which marks the card discarded and writes no claim. This keeps claim approval isolated from the slug-based legacy commit path.
...
| `POST` | `/api/kb/employers/{slug}/extract-claims` | New Stage 1-4 pipeline entry point (§5); returns proposed `IntentCard`s, does not write a `Claim`, but may create idempotent `Evidence` records and a known-employer organization entity while resolving the proposal |
| `POST` | `/api/kb/session/{session_id}/claims/{card_id}/commit` | Dedicated approval path; validates the claim card and writes the claim with `review_status="approved"` |
...
- `api/tests/test_ontology_extraction.py` — the Stage 1-4 pipeline against focused fixtures, asserting: mentions have valid offsets into the source text; ambiguous resolutions do not produce a claim; every produced claim has ≥1 evidence_id whose excerpt is a substring of the fixture source text (this is the automatable half of "evidence excerpts must be copied from source material, not generated" — verified by direct substring containment, not just schema shape).
- `api/tests/test_ontology_e2e_fixture.py` — the full extraction → IntentCard → dedicated approval path, plus the real Deloitte ambiguity case.
...
1. **Shipped:** `api/models_ontology.py` and validation tests.
2. **Shipped:** `EntityStore`, `ClaimStore`, `EvidenceStore`, and their tests.
3. **Shipped:** additive `SourceMetadata` normalization and source-ledger tests.
4. **Shipped dark:** the extraction pipeline and `/extract-claims` endpoint are behind `ontology.extraction_enabled: false`. The config is loaded at process import, so changing the flag takes effect after restart/redeploy, not hot reload.
5. **Shipped dark:** the SmartCanvas claim-card renderer and dedicated approval route. Pilot enablement is still pending.
6. **Next:** expand the gold evaluation set, run the pilot checks, and only then consider additional claim types such as `skill_requirement` or `compensation_observation`.

## 11. Risks and trade-offs

### 11.1 Risks this design inherits from the existing system without resolving

These are not new risks this design introduces — they are open, "Now"-priority items already tracked in `TODOS.md` for the existing system — but the new claim-review-and-commit workflow sits directly on top of them, so they should be treated as go/no-go conditions for enabling the pilot flag (`MIGRATION-PLAN.md` §3 Phase 3), not as pre-existing debt this design can safely ignore:

- **No real counsellor identity.** `_get_counsellor_id()` (`api/routers/session_router.py:64`) trusts the caller-supplied `X-Counsellor-ID` header. `Claim.review_status="approved"` will faithfully record *that* a claim was approved, but "approved by whom" has no verified identity behind it — the same gap `TODOS.md`'s "Counsellor RBAC" item already flags for every other commit path. This design does not add a new vulnerability, but it does add a new commit surface to the existing one.
- **No concurrency protection on structured KB writes.** `TODOS.md`'s "Basic multi-user edit protection" item is still open. `ClaimStore`/`EntityStore` inherit the same last-write-wins file model as every other store in this codebase. Two counsellors reviewing the same proposed-claim queue concurrently is not addressed here.
- **Langfuse egress scope grows.** `TODOS.md`'s "Langfuse data egress compliance audit" item is still open for existing KB/chat content. The new extraction prompts (§6) route through the same `_call_with_trace` tracing path and will additionally send source excerpts (potentially including alumni interview notes, compensation figures, and other PII-adjacent content pulled into `Evidence.excerpt`) to Langfuse. This is new content volume on an already-unresolved compliance question, not a new question — but it should be explicitly named to whoever owns that audit before the pilot flag goes on.

None of these block Phases 0-2 (models, stores, extension of the source ledger) — nothing in those phases is reachable by a counsellor or writes anything sensitive. They should block Phase 3 (real pilot data, real review-and-approve traffic) until each has an explicit owner decision, per `MIGRATION-PLAN.md` §6.

### 11.2 `contains_personal_data` is a decorative field in Milestone 1

`SourceMetadata.contains_personal_data: bool` exists on the model per the spec's field list, but Milestone 1 does not specify who sets it (manual counsellor input? an LLM classifier? a keyword heuristic?), when it's computed, or any behavior gated on it being `true`. As specified, it is a boolean that nothing reads. If PII-aware handling (e.g., excluding such sources from the extraction pipeline, or requiring a stricter authority tier) is actually needed before the pilot, that requires a follow-up design decision — this document only reserves the field, it does not implement a policy around it.

### 11.3 Other trade-offs
